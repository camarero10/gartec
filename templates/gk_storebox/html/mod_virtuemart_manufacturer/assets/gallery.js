window.addEvent('domready', function() {
	document.getElements('.gkNspPM-ManufacturerGallery').each(function(module) {
		if(!module.hasClass('active')) {
			module.addClass('active');
			gkPortalModeManufacturerGalleryInit(module);
		}
	});
});

var gkPortalModeManufacturerGalleryInit = function(module) {
	...
	if(module.getElement('.gkPrevBtn')) {
		module.getElement('.gkPrevBtn').addEvent('click', function(e) {
			...
			gkPortalModeManufacturerGalleryAnim(module, 'prev');
		});

		module.getElement('.gkNextBtn').addEvent('click', function(e) {
			e.preventDefault();
			module.setProperty('data-blank', 1);
			gkPortalModeManufacturerGalleryAnim(module, 'next');
		});
	}
		...
		module.getElements('.gkPagination li').each(function(el, i) {
			el.addEvent('click', function(e) {
				...
				if(i+1 != module.getProperty('data-current') * 1.0) {
					module.setProperty('data-blank', 1);
					gkPortalModeManufacturerGalleryAnim(module, i+1);
				}
			});
		});
	}
	...
	if(module.hasClass('gkAutoAnimation')) {
		setTimeout(function() {
			gkPortalModeManufacturerGalleryAutoAnim(module);
...
var gkPortalModeManufacturerGalleryAutoAnim = function(module) {
...
var gkPortalModeManufacturerGalleryAnim = function(module, dir) {
	...
	module.getElements('.gkImage').each(function(img) {
		if(img.hasClass('active')) {
			gkPortalModeManufacturerGalleryImgClass(img, 'active', false, 0);
			gkPortalModeManufacturerGalleryImgClass(img, '', true, 300);
		}
	});
	// pagination classes
	if(module.getElement('.gkPagination')) {
	...
	setTimeout(function() {
		module.getElements('.gkImage').each(function(img, i) {
			if(i >= (next - 1) * perPage && i < (next * perPage)) {
				gkPortalModeManufacturerGalleryImgClass(img, 'active', false, 0);
				gkPortalModeManufacturerGalleryImgClass(img, 'active show', true, 300);
			}
		});
	}, 300);
};

var gkPortalModeManufacturerGalleryImgClass = function(img, className, delay, time) {
	if(!delay) {
