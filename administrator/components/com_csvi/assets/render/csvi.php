 ...
 * @since    2.0
 */
class FOFRenderAssetsCsvi extends FOFRenderAbstract
{
...
ENDJS;
			...
			if ($show_filters)
		{
			JHtmlSidebar::setAction("index.php?option=" .
				$input->getCmd('option','com_csvi') . "&view=" .
				FOFInflector::pluralize($input->getCmd('view', 'cpanel'))
			);
		}
		...
		$html .= "\t" . '<input type="hidden" name="option" value="' . $input->getCmd('option', 'com_csvi') . '" />' . PHP_EOL;
		$html .= "\t" . '<input type="hidden" name="view" value="' . FOFInflector::pluralize($input->getCmd('view', 'cpanel')) . '" />' . PHP_EOL;
