...
class CsviHelperFileExportXmlGoogle
{
	...
	public function headerText($exportFields)
	{
		...
		$this->contents .= '<title>' . $this->settings->get('google_base.gb_title') . '</title>' . chr(10);
		$this->contents .= '<link>' . $this->settings->get('google_base.gb_link') . '</link>' . chr(10);
		$this->contents .= '<description>' . $this->settings->get('google_base.gb_description') . '</description>' . chr(10);
